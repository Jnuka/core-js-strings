...
function countVowels(str) {
  ...
  return str.split('').length > 0
    ? str
        .toLowerCase()
        .split('')
        .filter((char) => arrayVowels.includes(char)).length
    : 0;
}
...
function isPalindrome(str) {
  const replaceStr = str.toLowerCase().replace(/[?,! ]/g, '');
  return replaceStr === replaceStr.split('').reverse().join('');
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((letter) => {
      if (
        (letter.charCodeAt() > 64 && letter.charCodeAt() < 78) ||
        (letter.charCodeAt() > 96 && letter.charCodeAt() < 110)
      ) {
        return String.fromCharCode(letter.charCodeAt() + 13);
      }
      if (
        (letter.charCodeAt() > 76 && letter.charCodeAt() < 91) ||
        (letter.charCodeAt() > 109 && letter.charCodeAt() < 123)
      ) {
        return String.fromCharCode(letter.charCodeAt() - 13);
      }
      return letter;
...
function getCardId(value) {
  ...
  const suit = ['♣', '♦', '♥', '♠'];
  return (
    number.indexOf(value.slice(0, -1)) + suit.indexOf(value.slice(-1)) * 13
  );
}
